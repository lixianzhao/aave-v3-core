...
library DataTypes {
  struct ReserveData {
    ...
    // 流动性池自创立到更新时间戳之间的累计利率（贴现因子）
    uint128 liquidityIndex;
    //the current supply rate. Expressed in ray
    // 当前存款利率 LR = R（浮动利率贷款与固定利率贷款的利率加权平均值） * U（当前的利用率 D/S）
    uint128 currentLiquidityRate;
    //variable borrow index. Expressed in ray
    // the cumulated variable borrow index 累计变量借用指数
    // 浮动借款利率自流动性池建立以来的累计利率(贴现因子)
  ...
  struct ReserveConfigurationMap {
    ...
    //bit 58: borrowing is enabled 	是否可贷出
    //bit 59: stable rate borrowing enabled 是否可以以固定利率贷出
